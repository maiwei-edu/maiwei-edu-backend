...
const CourseAliyunPage = () => {
  ...
  useEffect(() => {
    document.title = "阿里云加密";
    dispatch(titleAction("阿里云加密"));
  }, []);
  ...
  const submit = () => {
    ...
    setLoading(true);
    course
      .aliyunHlsSubmit({ ids: selectedRowKeys })
      .then(() => {
        setLoading(false);
        message.success("成功");
        resetData();
      ...
      <div className="float-left j-b-flex  mt-30 mb-30">
        <div className="d-flex">
          <PerButton
            ...
            class=""
            icon={null}
            p="video.aliyun_hls.submit"
            onClick={() => submit()}
            disabled={null}
          />
          <OptionBar
            text="阿里云HLS加密"
            value="/system/videoHlsConfig?referer=%2Fcourse%2Fvod%2Fvideo%2Fhls%2Faliyun"
          ></OptionBar>
        </div>
        <div className="d-flex">
          <Input
...
export default CourseAliyunPage;
